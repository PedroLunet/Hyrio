class MessagesHandler {
    ...
    init() {
        ...
        if (!this.messagesList || !this.messageForm) {
            console.log('Messages elements not found, real-time messaging not initialized');
            return;
        }
    ...
    async sendMessage() {
        ...
        try {
            ...
            if (result.success) {
                ...
                this.showError('Failed to send message: ' + (result.error || 'Unknown error'));
            }
        } catch (error) {
    ...
    async checkForNewMessages() {
        try {
            const url = new URL('/actions/messages_action.php', window.location.origin);
